  ...
  const toggleTracking = useCallback(async () => {
    if (isTracking) {
      ...
      watchId.current = navigator.geolocation.watchPosition((position) => {
        ...
        const currentSpeed = unit === "knots" ? rawSpeed * 1.94384 : rawSpeed * 3.6;
    ...
    <CalculatorPage calc={calc} guideHtml={guideHtml} faqs={faqs} relatedArticles={relatedArticles}>
      <div className={cn("max-w-6xl mx-auto space-y-8", hudMode && "rotate-x-180 scale-x-[-1] opacity-90 brightness-150")}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {[
            { label: "Max Velocity", value: Math.round(maxSpeed), unit: unit, icon: Anchor, color: "text-blue-500" },
            { label: "Heading", value: heading ? `${Math.round(heading)}°` : "---", unit: "", icon: Navigation, color: "text-green-500" },
            { label: "Distance", value: (unit === "knots" ? distance*0.00054 : distance/1000).toFixed(2), unit: unit === "knots" ? "nm" : "km", icon: MapPin, color: "text-orange-500" },
            { label: "Marine GPS", value: "Active", unit: "", icon: Navigation, color: "text-red-500" },
          ].map((stat, i) => (
        ...
        <div className="surface-card p-6 border-border/30 bg-background rounded-2xl shadow-sm"><div className="flex items-center gap-3"><Info className="size-4 text-muted-foreground" /><h4 className="text-[10px] font-bold uppercase tracking-widest">Marine Speed Tracking</h4></div><p className="text-xs text-muted-foreground leading-relaxed">High-precision GPS speedometer for boating and sailing. Track your speed in knots or km/h, monitor your compass heading, and log nautical miles live on the water.</p></div>
      </div>
    </CalculatorPage>
  );
};
