...
import { Activity, Navigation, MapPin, Monitor, RotateCcw, Play, Square, Info, AlertCircle, TrendingUp } from "lucide-react";
...
const RunningSpeedometer = ({ guideHtml, faqs, relatedArticles }: { guideHtml?: string; faqs?: any[]; relatedArticles?: any[] }) => {
  const calc = useMemo(() => CALCULATORS.find(c => c.slug === "running-speedometer-test"), []);
  if (!calc) return null;
  ...
  const displayDistance = unit === "kmh" ? distance / 1000 : unit === "knots" ? distance * 0.000539957 : unit === "fpm" ? distance * 3.28084 : distance * 0.000621371;
  const distanceUnit = unit === "kmh" ? "km" : unit === "knots" ? "nm" : unit === "fpm" ? "ft" : "mi";
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start max-w-6xl mx-auto">
        ...
        <div className={cn("lg:col-span-8 space-y-8 order-1 lg:order-2", hudMode && "rotate-x-180 scale-x-[-1] opacity-90 brightness-150")}>
          <div className="surface-card p-6 md:p-12 bg-secondary/5 border-border/40 relative overflow-hidden shadow-sm flex flex-col items-center justify-between min-h-[500px] md:min-h-[600px] rounded-2xl">
            <Activity className="absolute inset-0 size-96 text-muted-foreground opacity-[0.03] -rotate-12 m-auto" />
            <div className="relative z-10 flex flex-col items-center gap-2">
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground">
                ...
                {isTracking ? "Live Run Tracking Active" : "Running Speedometer"}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[
              ...
              { label: "Avg Pace", value: Math.round(displayAvgSpeed), unit: unit, icon: Activity, color: "text-blue-500" },
              { label: "Run Distance", value: displayDistance.toFixed(2), unit: distanceUnit, icon: MapPin, color: "text-orange-500" },
              { label: "Run Timer", value: formatTime(elapsedTime), unit: "H:M:S", icon: Activity, color: "text-green-500" },
            ].map((stat, i) => (
          ...
          <div className="surface-card p-6 border-border/30 bg-background rounded-2xl shadow-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="text-[10px] font-bold uppercase tracking-widest">Running Analytics</h4>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">Track your running performance with high-fidelity GPS data. Monitor your sprint speed, average pace, and total distance covered. Ideal for outdoor training, track sessions, or casual jogging.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6 order-2 lg:order-1">
          <div className="surface-card p-6 md:p-8 space-y-8 bg-secondary/5 border-border/40 relative overflow-hidden group">
            ...
            <div className="space-y-1 relative z-10">
              <h3 className="text-sm font-bold tracking-tight">Run Settings</h3>
          ...
          <div className="surface-card p-6 border-signal/20 bg-signal/[0.03] text-signal relative overflow-hidden group">
            ...
            <div className="flex gap-4 items-start relative z-10">
              ...
              <div className="space-y-1">
                <h4 className="text-[10px] font-bold uppercase tracking-wider opacity-80">Training Note</h4>
                <p className="text-xs leading-relaxed font-medium">
                  GPS accuracy is highest in open environments. For track running, try to maintain a clear line of sight to the sky.
                </p>
              </div>
            </div>
          </div>
...
export default RunningSpeedometer;
