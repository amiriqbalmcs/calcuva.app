...
const PASSAGES = [
  ...
  "Code is like humor. When you have to explain it, it’s bad. Programming isn't about what you know; it's about what you can figure out. The best way to predict the future is to invent it.",
  "Stay hungry, stay foolish. Innovation is saying no to a thousand things. Simple can be harder than complex: you have to work hard to get your thinking clean to make it simple.",
  ...
  "If life were predictable it would cease to be life, and be without flavor. Life is what happens when you're making other plans. Spread love everywhere you go. Let no one ever come to you without leaving happier.",
  "Tell me and I forget. Teach me and I remember. Involve me and I learn. It is during our darkest moments that we must focus to see the light. Whoever is happy will make others happy too."
];

const TypingSpeedTest = ({ guideHtml, faqs, relatedArticles }: { guideHtml?: string; faqs?: any[]; relatedArticles?: any[] }) => {
  // Add custom animation for caret
  useEffect(() => {
  ...
  const [attempts, setAttempts] = useState<{wpm: number, accuracy: number, date: string}[]>([]);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const acc = val.length === 0 ? 100 : Math.round(((val.length - errors) / val.length) * 100);
    setAccuracy(acc);
    ...
    if (val.length === passage.length) {
      ...
      for (let i = 0; i < passage.length; i++) {
        if (val[i] !== passage[i]) finalErrors++;
      }
      const finalAccuracy = Math.round(((passage.length - finalErrors) / passage.length) * 100);
  ...
  const characters = useMemo(() => {
    return passage.split("").map((char, i) => {
      let charClass = "text-muted-foreground/30";
      let isCaret = i === userInput.length;

      if (i < userInput.length) {
        charClass = userInput[i] === passage[i] ? "text-foreground" : "text-red-500 bg-red-500/10 rounded-sm";
      }

      return (
        <span key={i} className="relative inline-block">
          <span className={cn("transition-colors duration-75", charClass)}>
            {char}
          </span>
          {isCaret && (
            <span className="absolute left-0 top-0 bottom-0 w-0.5 bg-signal animate-caret-blink" />
          )}
        </span>
      );
    });
          ...
          <div className="flex items-center justify-between px-4">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className="p-2.5 rounded-xl hover:bg-secondary transition-all border border-border/20 bg-background flex items-center gap-2 hidden sm:flex"
              >
          ...
          <div className="surface-card border-border/40 bg-secondary/5 rounded-2xl shadow-sm min-h-[500px] relative overflow-hidden flex flex-col items-center justify-center p-6 md:p-16 text-center group transition-all duration-500">
            ...
            {gameState === "PLAYING" && (
              <div className="space-y-12 w-full max-w-4xl relative z-10">
                <div className="text-xl md:text-3xl font-medium leading-relaxed md:leading-[1.6] text-left select-none font-sans tracking-tight whitespace-pre-wrap">
                  {characters}
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className={cn(
                    "flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground/40",
                    userInput.length > 0 ? "opacity-0" : "animate-pulse"
                  )}>
                  ...
                  <button 
                    onClick={initGame}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-secondary/20 transition-all text-[10px] font-bold uppercase tracking-widest text-muted-foreground group"
                  >
                    <RefreshCcw className="size-3 group-hover:rotate-180 transition-transform duration-500" />
                    Reset Test
                  </button>
                </div>
              </div>
            ...
            {gameState === "RESULT" && (
              <div className="space-y-12 animate-in zoom-in-95 duration-500 w-full max-w-2xl z-10">
                  ...
                  <div className="grid grid-cols-2 gap-8 sm:gap-12">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-baseline justify-center gap-2">
                        <h2 className="text-7xl sm:text-9xl font-mono font-bold tracking-tighter tabular-nums">{wpm}</h2>
                        <span className="text-xl sm:text-2xl font-bold text-signal/40 uppercase">WPM</span>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-blue-500">Accuracy</p>
                      <div className="flex items-baseline justify-center gap-2">
                        <h2 className="text-7xl sm:text-9xl font-mono font-bold tracking-tighter tabular-nums">{accuracy}</h2>
                        <span className="text-xl sm:text-2xl font-bold text-blue-500/40 uppercase">%</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  <button
                    onClick={initGame}
                    className="h-12 px-8 rounded-xl bg-foreground text-background font-bold uppercase tracking-widest text-[10px] hover:scale-105 transition-all w-full sm:w-auto flex items-center justify-center gap-2"
                  >
            ...
            <div className="surface-card p-6 border-border/40 bg-background rounded-2xl shadow-sm relative overflow-hidden group">
              ...
              <div className="space-y-1.5">
                {attempts.length > 0 ? (
                  attempts.map((attempt, i) => (
                    <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-secondary/30 border border-border/10">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[7px] text-muted-foreground/60">{attempt.date}</span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="font-mono font-bold text-xs">{attempt.wpm} WPM</span>
                        <span className={cn("text-[7px] font-bold", attempt.accuracy >= 95 ? "text-signal" : "text-muted-foreground/60")}>
                  ...
                  [0, 1, 2, 3, 4].map((i) => (
                    <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-secondary/5 border border-border/5 opacity-50">
                      <span className="text-[9px] font-bold text-muted-foreground/40 uppercase">Trial --</span>
                      <span className="font-mono font-bold text-xs text-muted-foreground/20">---</span>
                    </div>
                  ))
                )}
              </div>
          ...
          {calc.howTo && (
            <div className="pt-8 border-t border-border/40">
              <div className="mb-6 text-center sm:text-left">
                <h3 className="text-lg font-bold tracking-tight">Typing Speed Improvement Guide</h3>
                <p className="text-xs text-muted-foreground uppercase tracking-widest font-bold opacity-60">Master Your Keyboard</p>
              </div>
              <HowToGuide
                id="how-to-use"
