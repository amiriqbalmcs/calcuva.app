...
import { Navigation, MapPin, Monitor, RotateCcw, Play, Square, Activity, Info, AlertCircle, TrendingUp } from "lucide-react";
...
const HorseSpeedometer = ({ guideHtml, faqs, relatedArticles }: { guideHtml?: string; faqs?: any[]; relatedArticles?: any[] }) => {
  const calc = useMemo(() => CALCULATORS.find(c => c.slug === "horse-riding-speed-tracker"), []);
  if (!calc) return null;
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start max-w-6xl mx-auto">
        ...
        <div className={cn("lg:col-span-8 space-y-8 order-1 lg:order-2", hudMode && "rotate-x-180 scale-x-[-1] opacity-90 brightness-150")}>
          <div className="surface-card p-6 md:p-12 bg-secondary/5 border-border/40 relative overflow-hidden shadow-sm flex flex-col items-center justify-between min-h-[500px] md:min-h-[600px] rounded-3xl">
            <Navigation className="absolute inset-0 size-96 text-muted-foreground opacity-[0.03] -rotate-12 m-auto" />
            <div className="relative z-10 flex flex-col items-center gap-2">
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground">
                ...
                {isTracking ? "Equestrian Tracking Active" : "Horse Riding Speedometer"}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[
              ...
              { label: "Trail Distance", value: displayDistance.toFixed(2), unit: distanceUnit, icon: MapPin, color: "text-orange-500" },
              { label: "Ride Timer", value: formatTime(elapsedTime), unit: "H:M:S", icon: Activity, color: "text-green-500" },
            ].map((stat, i) => (
          ...
          <div className="surface-card p-6 border-border/30 bg-background rounded-2xl shadow-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="text-[10px] font-bold uppercase tracking-widest">Equestrian Tracking</h4>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">Monitor your horse's velocity and trail performance with precision. Track real-time speed in Trot, Canter, or Gallop and record your total riding distance. Keep your phone in a secure saddlebag or pocket with GPS enabled.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6 order-2 lg:order-1">
          <div className="surface-card p-6 md:p-8 space-y-8 bg-secondary/5 border-border/40 relative overflow-hidden group">
            <Navigation className="absolute -bottom-6 -left-6 size-32 text-muted-foreground/5 -rotate-12 transition-transform group-hover:rotate-0 duration-700" />

            <div className="space-y-1 relative z-10">
              <h3 className="text-sm font-bold tracking-tight">Ride Settings</h3>
            ...
            <div className="flex gap-4 items-start relative z-10">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-relaxed font-medium">
                  Ensure GPS is enabled. Trail coverage may vary depending on terrain and tree density.
                </p>
              </div>
            </div>
          </div>
...
export default HorseSpeedometer;
