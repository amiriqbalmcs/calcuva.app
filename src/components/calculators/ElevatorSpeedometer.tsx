...
import { ArrowUpCircle, Navigation, MapPin, Monitor, RotateCcw, Play, Square, Activity, Info, AlertCircle, TrendingUp } from "lucide-react";
...
const ElevatorSpeedometer = ({ guideHtml, faqs, relatedArticles }: { guideHtml?: string; faqs?: any[]; relatedArticles?: any[] }) => {
  const calc = useMemo(() => CALCULATORS.find(c => c.slug === "high-speed-elevator-test"), []);
  if (!calc) return null;

  const [unit, setUnit] = useState<"fpm" | "mph" | "kmh" | "knots">("fpm");
  ...
  const toggleTracking = useCallback(async () => {
    if (isTracking) {
      ...
      watchId.current = navigator.geolocation.watchPosition((position) => {
        ...
        const filteredSpeed = rawSpeed > 0.1 ? rawSpeed : 0; // Lower threshold for elevators as they can be slow
        setSpeed(filteredSpeed);
        if (filteredSpeed > 0) setMaxSpeed(prev => Math.max(prev, filteredSpeed));
        setAltitude(position.coords.altitude);

        if (lastPos.current && filteredSpeed > 0.1) {
          const R = 6371e3;
  ...
  const displayDistance = unit === "fpm" ? distance * 3.28084 : unit === "kmh" ? distance / 1000 : unit === "knots" ? distance * 0.000539957 : distance * 0.000621371;
  const distanceUnit = unit === "fpm" ? "ft" : unit === "kmh" ? "km" : unit === "knots" ? "nm" : "mi";
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start max-w-6xl mx-auto">
        ...
        <div className={cn("lg:col-span-8 space-y-8 order-1 lg:order-2", hudMode && "rotate-x-180 scale-x-[-1] opacity-90 brightness-150")}>
          <div className="surface-card p-6 md:p-12 bg-secondary/5 border-border/40 relative overflow-hidden shadow-sm flex flex-col items-center justify-between min-h-[500px] md:min-h-[600px] rounded-3xl">
            <ArrowUpCircle className="absolute inset-0 size-96 text-muted-foreground opacity-[0.03] -rotate-12 m-auto" />
            <div className="relative z-10 flex flex-col items-center gap-2">
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground">
                ...
                {isTracking ? "Elevator Telemetry Active" : "Elevator Speed Test"}
              </div>
            </div>
            ...
            <div className="relative z-20 flex items-center justify-center gap-2 md:gap-8 w-full border-t border-border/10 pt-8">
              <button onClick={() => setHudMode(!hudMode)} className={cn("h-10 md:h-12 px-3 md:px-6 rounded-xl border flex items-center gap-2 font-bold text-[10px] uppercase transition-all", hudMode ? "bg-blue-500 text-white shadow-md border-blue-600" : "bg-background text-muted-foreground border-border/60 hover:bg-secondary")}><Monitor className="size-3 md:size-4" /><span className="hidden sm:inline">HUD Mode</span></button>
              <button onClick={toggleTracking} className={cn("h-12 md:h-16 px-6 md:px-14 rounded-xl border flex items-center gap-3 font-bold text-xs uppercase shadow-lg transition-all hover:scale-105 active:scale-95", isTracking ? "bg-red-500 text-white border-red-600" : "bg-foreground text-background border-foreground")}>{isTracking ? <Square className="size-4 fill-current" /> : <Play className="size-4 fill-current" />}<span>{isTracking ? "STOP" : "START GPS"}</span></button>
              <button onClick={() => { setMaxSpeed(0); setDistance(0); setElapsedTime(0); setMaxG(0); }} className="h-10 md:h-12 px-3 md:px-6 rounded-xl border border-border/60 bg-background text-muted-foreground flex items-center gap-2 font-bold text-[10px] uppercase transition-all hover:bg-secondary"><RotateCcw className="size-3 md:size-4" /><span className="hidden sm:inline">Reset</span></button>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[
              { label: "Max Vertical Speed", value: Math.round(displayMaxSpeed), unit: unit, icon: TrendingUp, color: "text-red-500" },
              ...
              { label: "Vertical Travel", value: displayDistance.toFixed(2), unit: distanceUnit, icon: MapPin, color: "text-orange-500" },
            ].map((stat, i) => (
          ...
          <div className="surface-card p-6 border-border/30 bg-background rounded-2xl shadow-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="text-[10px] font-bold uppercase tracking-widest">Elevator Velocity Tracking</h4>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">Measure the vertical speed of any elevator in Feet per Minute (FPM) or MPH. This tool also tracks the physical G-forces during acceleration and braking, providing a complete performance profile of high-speed skyscraper lifts.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6 order-2 lg:order-1">
          <div className="surface-card p-6 md:p-8 space-y-8 bg-secondary/5 border-border/40 relative overflow-hidden group">
            <ArrowUpCircle className="absolute -bottom-6 -left-6 size-32 text-muted-foreground/5 -rotate-12 transition-transform group-hover:rotate-0 duration-700" />

            <div className="space-y-1 relative z-10">
              <h3 className="text-sm font-bold tracking-tight">Lift Settings</h3>
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Session Parameters</p>
            </div>
            ...
            <div className="flex gap-4 items-start relative z-10">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-relaxed font-medium">
                  Vertical GPS tracking works best in modern buildings with large glass windows or high signal penetration.
                </p>
              </div>
            </div>
          </div>
...
export default ElevatorSpeedometer;
